...
from android_world.agents import t3a,m3a,UI_TARS_M3A, RAC_Agent
...
def _find_adb_directory() -> str:
  ...
  potential_paths = [
      ...
      os.path.expanduser("C:/Users/AaronQin/AppData/Local/Android/Sdk/platform-tools/adb.exe"),

      # Set ADB_PATH/ANDROID_HOME if adb is not discoverable automatically.
  ]
...
def _main() -> None:
  ...
  adb_utils.launch_app(_APP.value, env.controller)
  # agent = t3a.T3A(env, infer.Gpt4Wrapper('openai/gpt-4'))
  # agent = t3a.T3A(env, infer.QwenWrapper('qwen2.5-vl-3b-instruct'))
  # agent = m3a.M3A(env, infer.QwenWrapper('qwen2.5-vl-7b-instruct'))
  agent = m3a.M3A(env, infer.Gpt4Wrapper('anthropic/claude-sonnet-4'))
  # agent = UI_TARS_M3A.SRoA(env, infer.UiTarsWrapper('ui-tars'))
  # agent = RAC_Agent.RAC(env, cloud_llm=infer.Gpt4Wrapper('anthropic/claude-sonnet-4'), local_llm=infer.QwenWrapper('qwen2.5-vl-3b-instruct'), uri=config.Neo4j_URI, auth=config.Neo4j_AUTH)
