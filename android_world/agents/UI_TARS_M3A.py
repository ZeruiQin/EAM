...
import re
PROMPT_PREFIX = (
    r"""You are a GUI agent. You are given a user instruction with screenshots and a list of task related elements in the current screenshots. You need to following the task instruction by performing the next action to complete the task. 
...
ACTION_SELECTION_PROMPT_TEMPLATE = (
    ...
    ' following the reason why you do that. '
)
...
class SRoA(base_agent.EnvironmentInteractingAgent):
  ...
  def step(self, goal: str) -> base_agent.AgentInteractionResult:
    ...
    # action_output = extract_action_and_coordinates(action_output)
    # print(action_output)
    # print(raw_response)
...
Action: {{"action_type": "status", "goal_status": "infeasible"}}"""
    ...
    reason, action = m3a_utils.parse_reason_action_output_coordinate(action_output,width,height)
