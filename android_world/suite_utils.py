...
#     task.initialize_task(env)
#     _log_and_print('Running task %s with goal "%s"', task.name, task.goal)
#     package, app = get_app_info(task.name)
...
#
#     def set_kb(self, package_name, task_name):
#         """设置知识库文件路径到环境变量"""
#         kb_file = self.kb_directory / f"ui_automation_paths_{package_name}.json"
#         os.environ["TASK"] = str(task_name)
#         if kb_file.exists():
#             os.environ["CURRENT_KB_FILE"] = str(kb_file)
#             return True
#         return False
#
#     @staticmethod
#     def get_kb():
#         """获取当前知识库文件路径"""
#         return os.environ.get("CURRENT_KB_FILE")
#
# def set_database_index(app_name):
#     """设置知识库文件路径到环境变量"""
#     os.environ["DATABASE"] = str(app_name)
#     return True
#
#
# def get_all_tasks_from_exploration_output(exploration_output_path):
#     """
#     遍历exploration_output文件夹，提取所有包的任务名称
#     """
#     tasks_dict = {}
#     all_tasks = []
#
#     exploration_path = Path(exploration_output_path)
#
#     # 遍历所有包文件夹
#     for package_dir in exploration_path.iterdir():
#         if package_dir.is_dir():
#             package_name = package_dir.name
#             tasks = []
#
#             # 遍历包文件夹下的所有任务文件夹
#             for task_dir in package_dir.iterdir():
...
#
#             if tasks:
#                 tasks_dict[package_name] = tasks
#
#     return tasks_dict, all_tasks
#
# def _run_task_suite(
#     suite: Suite,
#     run_episode: Callable[[task_eval.TaskEval], episode_runner.EpisodeResult],
...
#   full_episode_data = []
#   correct, total = 0, 0
#   kb_manager = KnowledgeBaseManager("task_explorer/utils")
#   task_dict, all_tasks = get_all_tasks_from_exploration_output("artifacts/trajectories")
#   for name, instances in suite.items():
#     if name not in all_tasks:
#         continue
#     msg = 'Running task: ' + name
#     _log_and_print(msg + '\n' + '=' * len(msg))
#     package, app = get_app_info(name)
#     kb_manager.set_kb(package, name)
#     app = app.replace(" ", "-")
#     set_database_index(app)
#     for i, instance in enumerate(instances):
#       instance_name = (
#           instance.name + checkpointer_lib.INSTANCE_SEPARATOR + str(i)
#       )
